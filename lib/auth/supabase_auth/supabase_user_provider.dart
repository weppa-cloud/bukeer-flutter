...
class BukeerSupabaseUser extends BaseAuthUser {
  BukeerSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> bukeerSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BukeerSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
