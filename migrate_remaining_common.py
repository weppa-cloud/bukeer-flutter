...
# Get all files that have remaining common FFAppState references
result = subprocess.run([
    'grep', '-r', '-l', 
    'FFAppState()\.searchStringState\|FFAppState()\.typeProduct', 
    'lib/', '--include=*.dart'
...
print(f"Found {len(files_to_process)} files with searchStringState/typeProduct references")

# Define replacements for common remaining patterns
replacements = [
    (r'FFAppState\(\)\.searchStringState', 'context.read<UiStateService>().searchQuery'),
    (r'FFAppState\(\)\.typeProduct', 'context.read<UiStateService>().selectedProductType'),
]
